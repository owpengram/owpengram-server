...
func TestDonationsCreateChainStartsWatcherWithoutRestart(t *testing.T) {
	...
	// Assert on THIS chain's own deposit row: another chain row may point at
	// the same local node and credit the same transaction, which would
	// otherwise satisfy a plain balance check without proving anything about
	// the dynamically started watcher.
	deadline := time.Now().Add(10 * time.Second)
	for {
		deposit, found := depositForChain(t, ctx, donationStore, donor.ID, chainKey)
		if found && deposit.Status == domain.DonationDepositCredited && deposit.StarsCredited > 0 {
			t.Logf("credited via dynamically started watcher: %d Stars on %s", deposit.StarsCredited, chainKey)
			return
		}
		if time.Now().After(deadline) {
			t.Fatalf("timed out waiting for the dynamically created chain's watcher to credit the deposit (found=%v, %+v) -- CreateChain did not actually start a watcher", found, deposit)
		}
