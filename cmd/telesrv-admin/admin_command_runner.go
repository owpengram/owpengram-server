...
// close.
//
// The runner also makes command_id strictly idempotent: the same command_id
// always produces the same outcome, a replayed command returns the stored
// result with already_executed set, and a command_id reused for a DIFFERENT
// request is refused outright instead of silently re-running the mutation.
//
...
func (s *server) runOperatorCommand(ctx context.Context, meta admin.CommandMeta, action string, params any, password string, fn func(context.Context, pgx.Tx) (admin.CommandResult, error)) (admin.CommandResult, error) {
	...
	_, err = scanStoredOperatorCommand(tx.QueryRow(ctx, `
