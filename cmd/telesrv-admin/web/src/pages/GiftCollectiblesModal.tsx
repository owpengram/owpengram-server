...
import type { CommandResult, StarGiftCatalogRow, StarGiftCollectibleAttributeRow, StarGiftCollectiblePreview } from "../types";
...
export function GiftCollectiblesModal({ gift, onClose, onPublished }: { gift: StarGiftCatalogRow; onClose: () => void; onPublished: () => void }) {
  ...
  async function validate() {
    ...
    try { setPreview(await api.publishGiftCollectibles(buildForm(false))); }
  ...
  async function publish() {
    ...
    try {
      await api.publishGiftCollectibles(buildForm(true, preview.command_id));
      onPublished(); onClose();
    } catch (err) { setError(errorMessage(err)); }
    finally { setBusy(false); }
  }
