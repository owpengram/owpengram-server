...
export const api = {
  ...
  premiumPlans: () => request<PremiumPlanListResponse>("/api/premium/plans"),
  premiumPayment: (id: string) => request<PremiumPaymentIntent>(`/api/premium/payments/${encodeURIComponent(id)}`),
  ...
  reviewModerationAppeal: (caseID: number, appealID: number, payload: Record<string, unknown>) =>
	...
	starGiftCatalog: () => request<StarGiftCatalogListResponse>("/api/star-gift-catalog"),
	starGiftCatalogAnimationURL: (giftID: string) => `/api/star-gift-catalog/${encodeURIComponent(giftID)}/animation`,
	giftAnimation: (giftID: string) => request<Record<string, unknown>>(`/api/star-gift-catalog/${encodeURIComponent(giftID)}/animation`),
	createStarGiftCatalogEntry: (form: FormData) => request<CommandResult>("/api/actions/create-star-gift-catalog-entry", { method: "POST", body: form }),
	giftCollectibles: (giftID: string) => request<StarGiftCollectiblePreview>(`/api/star-gift-catalog/${encodeURIComponent(giftID)}/collectibles`),
